...
import { Play, Pause, Wind, Brain, Waves, Loader2, Volume2, StopCircle, Layers, RefreshCcw, Smartphone, Music, Headphones } from 'lucide-react';
import { getCognitiveShuffleWords, playTextToSpeech } from '../services/gemini';
...
const RelaxationTools: React.FC = () => {
  ...
  const [nsdrLoading, setNsdrLoading] = useState(false);
  const [nsdrPlaying, setNsdrPlaying] = useState(false);
  ...
  useEffect(() => {
    if (mode === 'SHUFFLE' && isActive) {
        if (shuffleWords.length === 0) {
            ...
            getCognitiveShuffleWords().then(words => {
        ...
        const interval = setInterval(() => {
            setCurrentWordIndex(prev => (prev + 1) % shuffleWords.length);
        }, 8000); // New word every 8 seconds
        
        return () => clearInterval(interval);
    }
  }, [isActive, mode, shuffleWords]);
  ...
  // NSDR Playback
  const handlePlayNSDR = async (script: string, id: string) => {
    if (activeNSDR === id && nsdrPlaying) {
        setNsdrPlaying(false);
        setActiveNSDR(null);
        return;
    }
    ...
    try {
        ...
        console.error("NSDR Playback failed", e);
        setActiveNSDR(null);
    } finally {
        setNsdrLoading(false);
    }
  };
      ...
      <div className="flex-1 flex flex-col items-center justify-center min-h-[50vh] w-full relative">
        ...
        {mode === 'SHUFFLE' && (
            <div className="text-center px-6 animate-fade-in w-full max-w-sm">
                {shuffleLoading ? (
                    <div className="text-stone-500 animate-pulse flex flex-col items-center gap-4">
                        <div className="w-12 h-12 rounded-full border-2 border-stone-800 border-t-orange-500 animate-spin"></div>
                        Generating imagery...
                    </div>
                ) : (
                    <>
                        ...
                        <div className={`text-4xl font-serif text-orange-100 transition-all duration-1000 transform ${isActive ? 'opacity-100 scale-100 translate-y-0' : 'opacity-50 blur-sm scale-95 translate-y-4'}`}>
                            {isActive ? shuffleWords[currentWordIndex] || "Ready" : "Ready"}
                        </div>
        ...
        {/* NSDR UI */}
        {mode === 'NSDR' && (
            <div className="w-full max-w-sm px-4 animate-fade-in space-y-4">
                {activeNSDR ? (
                     <div className="text-center relative py-12">
                        {/* Visualizer for active session */}
                        <div className="absolute inset-0 flex items-center justify-center -z-10 opacity-30 pointer-events-none">
                            <div className="w-64 h-64 bg-teal-900/20 rounded-full blur-3xl animate-pulse"></div>
                            <div className="absolute w-48 h-48 border border-teal-800/30 rounded-full animate-[ping_4s_linear_infinite]"></div>
                            <div className="absolute w-32 h-32 border border-teal-800/50 rounded-full animate-[ping_4s_linear_infinite_1s]"></div>
                        </div>

                        <div className="mb-6">
                            <span className="text-xs font-bold text-teal-400 tracking-widest uppercase">Now Playing</span>
                            <h3 className="text-2xl font-serif text-stone-200 mt-2">{NSDR_SESSIONS.find(s => s.id === activeNSDR)?.title}</h3>
                        </div>

                        <div className="flex justify-center">
                            <button 
                                onClick={() => { setNsdrPlaying(false); setActiveNSDR(null); }}
                                className="p-4 rounded-full bg-stone-800/80 border border-stone-700 hover:bg-stone-700 text-stone-300 transition-all"
                            >
                                <StopCircle size={32} />
                            </button>
                        </div>
                        <p className="mt-8 text-xs text-stone-500">Close your eyes. Follow the voice.</p>
                     </div>
                ) : (
                    <>
                        <div className="text-center mb-6">
                            <h3 className="text-lg font-medium text-stone-200">Non-Sleep Deep Rest</h3>
                            <p className="text-xs text-stone-500 mt-1">Deep relaxation protocols to restore dopamine.</p>
                        </div>
                        {NSDR_SESSIONS.map((session) => (
                            <button
                                key={session.id}
                                onClick={() => handlePlayNSDR(session.script, session.id)}
                                disabled={nsdrLoading}
                            ...
                            >
                                ...
                                <div className="absolute bottom-4 right-4 text-stone-700 group-hover:text-teal-500 transition-colors">
                                    {nsdrLoading && activeNSDR === session.id ? <Loader2 size={18} className="animate-spin" /> : <Play size={18} />}
                                </div>
                            </button>
                        ))}
                    </>
