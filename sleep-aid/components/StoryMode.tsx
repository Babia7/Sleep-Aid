...
import { BookOpen, Volume2, StopCircle, Feather, ArrowLeft, Clock } from 'lucide-react';
  ...
  const handleReadAloud = async (text: string) => {
    ...
    try {
      ...
      console.error("Audio playback failed", e);
        ...
        setTimeout(() => setPlaying(false), 5000); // Reset for demo
    }
  };
  ...
  const Reader = ({ title, content }: { title: string, content: string }) => (
    <div className="animate-fade-in h-full flex flex-col">
       <div className="flex justify-between items-center mb-6">
           ...
           <button 
            onClick={() => handleReadAloud(content)}
           ...
           >
               {playing ? <StopCircle size={14} /> : <Volume2 size={14} />}
